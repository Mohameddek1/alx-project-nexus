# Multi-Business Finance System - Complete Backend Architecture
...
```
USER RELATIONSHIPS:
User ── owns ──► Business (1:many)
User ── has roles in ──► UserBusinessRole (1:many)
User ── creates ──► TransactionCategory (1:many)
User ── creates ──► Transaction (1:many)
User ── performs ──► AuditLog (1:many)

BUSINESS RELATIONSHIPS:
Business ── has users ──► UserBusinessRole (1:many)
Business ── has categories ──► TransactionCategory (1:many)
Business ── has transactions ──► Transaction (1:many)
Business ── has audit logs ──► AuditLog (1:many)

TRANSACTION RELATIONSHIPS:
TransactionCategory ── categorizes ──► Transaction (1:many)
Transaction ── belongs to ──► Business (many:1)
Transaction ── belongs to ──► TransactionCategory (many:1)

INTER-BUSINESS RELATIONSHIPS:
Business ── sends ──► InterBusinessTransaction (1:many)
Business ── receives ──► InterBusinessTransaction (1:many)
InterBusinessTransaction ── has balance ──► InterBusinessBalance (many:1)
InterBusinessTransaction ── has schedule ──► RepaymentSchedule (1:many)

SHARED EXPENSE RELATIONSHIPS:
SharedExpense ── split between ──► SharedExpenseSplit (1:many)
SharedExpense ── paid by ──► Business (many:1)
SharedExpenseSplit ── belongs to ──► Business (many:1)
```
...
## 🔒 USER & PERMISSIONS MANAGEMENT FLOW
...
This comprehensive diagram collection shows the complete backend system architecture from authentication through all features, with clear entity relationships and simplified flow diagrams for the entire multi-business finance system.
